...
function Banco(){
	...
	this.buscaColecao = function(valor, stmt, mode){
		...
		if(mode == 'id'){
			stmt.text = "SELECT numCol FROM colecao WHERE nomeCol='"+valor+"';";
		}else if( (valor == null) && (mode == 'lista') ){
			stmt.text = "SELECT * FROM colecao ORDER BY numCol ASC;";
		}
		...
		colecoes = stmt.getResult();
		var lista_colecao = [];
		$.each(colecoes.data, function(i){
			lista_colecao[colecoes.data[i].numCol] = [colecoes.data[i].nomeCol];			
		});
	...
	this.buscaLinha = function(stmt, colecao, mode){
		...
		if(mode == 'id'){
			...
			stmt.text = "select * from linha where numCol ="+colecao+" ORDER BY numLin";
		}
	...
	this.insert = function(valores, tabela){
		...
		switch(tabela){
			...
			case 'linha':		
				try {	
					var idcolecao = parseInt(this.buscaColecao(valores[2], sql, 'id'));
