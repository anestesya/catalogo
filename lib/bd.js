...
function Banco(){
	...
	this.buscaColecao = function(valor){
		sql.text = "SELECT numCol FROM colecao WHERE nomeCol='"+valor+"';";
		sql.execute();
	
		var colID = sql.getResult();
	...
	this.buscaLinha = function(){
		sql.text = "SELECT numLin FROM linha;";
		sql.execute();
	
		var linID = sql.getResult(), linhaID;
		...
		switch(tabela){
			...
			case 'linha':		
				try {
					...
					sql.parameters['@numCol'] = parseInt(this.buscaColecao(valores[2]));
					sql.execute();					
				}catch(e) {
					this.dbError(e);
				}
			...
			case 'modelo':
				try {
					...
					sql.parameters['@numMod'] = parseInt(valores[1]);
					sql.parameters['@imggMod'] = valores[0];
					sql.parameters['@imgpMod'] = valores[2];
					sql.parameters['@corNumMod'] = parseInt(valores[1]);
					sql.parameters['@numLinMod'] = parseInt(valores[1]);
					sql.execute();					
				}catch(e) {
					this.dbError(e);
				}
