...
function Banco(){
	...
	this.dbError = function(e){
		$('<p class="alert">Database Error - msg:'+e.message+' - Detalhes: '+e.details+ ' - Operação: '+e.operation+'</p>').appendTo('body');
		setTimeout(function(){
			$('.alert').fadeOut('slow');
			$('.alert').remove();
		}, 6000);
	...
	//###########################################################################################
	//função que imprime mensgem de suceess na tela caso os dados sejam manipulados corretamente
	this.sucesso = function(){
		$('<p class="info">Dados cadastrados com sucesso!</p>').appendTo('body');
		setTimeout(function(){
			$('.info').fadeOut('slow');
			$('.info').remove();
		}, 1000);
	}//fim da msg de sucesso()
	...
	this.buscaLinha = function(stmt, colecao, mode){
		...
		if((mode == null || mode == 'undefined') && (typeof(colecao) == 'string')){
			stmt.text = "select numLin, nomeLin from linha where numCol="+colecao+" ORDER BY numLin";
		}else if(mode == 'id'){
		...
		if(mode == 'id'){
			var linID = stmt.getResult();
			linID = linID.data[0].numLin;
			return linID;
		}else {
	...
	//#################################################################################
	//busca a lista de cores disponíveis.
	this.getCores = function(stmt, cor, mode){
		if(stmt == null || stmt == 'undefined'){
			var cores, st = new air.SQLStatement();
			stmt = st;
	    	cnx.open(db);
			stmt.sqlConnection = cnx;
		} 
		...
		if(cor == null || cor == 'undefined'){
			stmt.text="select * from cores order by numCor";
		}else {
			stmt.text="select numCor from cores where nomeCor='"+cor+"'";
		}
		...
		if(mode == 'id'){
			cores = stmt.getResult();
			cores = cores.data[0].numCor;
			return cores;
		}

		cores = stmt.getResult();	
		return cores;
	}//fim do método getCores();
	...
	this.insert = function(valores, tabela){
		...
		switch(tabela){
			...
			case 'modelo':
				try {
					c = this.getCores(sql, 'Branca', 'id');
					l = this.buscaLinha(sql, 'Moira', 'id');
					...
					sql.parameters['@corNumMod'] = parseInt(c);
					sql.parameters['@numLinMod'] = parseInt(l);
					sql.execute();	
					this.sucesso();				
				}catch(e) {
					this.dbError(e);
			...
			case 'detalhes':
				try {
					li = this.buscaLinha(sql, valores[3], 'id');
					...
					sql.parameters['@numLinDet'] = parseInt(li);
					sql.execute();	
					this.sucesso();				
				}catch(e) {
					this.dbError(e);
			...
			case 'tamanho':
				try {
					lin = this.buscaLinha(sql, valores[3],'id');
					...
					sql.parameters['@numLinTa'] = parseInt(lin);
					sql.execute();
					this.sucesso();					
				}catch(e) {
					this.dbError(e);
